    ...
    public static void runChatEvent(PlayerEntity player, String message) {
        new Thread(() -> {
            try {
                ...
                else if (message.equalsIgnoreCase(SplitSelf.translate("chat.splitself.prompt.tethered").getString())) {playerManager.broadcast(Text.literal("<" + player.getName().getString() + "> " + SplitSelf.translate("chat.splitself.response.name").getString()), false);}
