...
public class LanguageEntryMixin {
    ...
    String languageCode;
    @Unique
    private static final Set<String> SUPPORTED_LANGUAGE_CODES = Set.of("en_us", "hu_hu", "it_it", "pl_pl", "pt_br", "ru_ru", "sv_se");
    @Unique
    private static final Set<String> SEMI_SUPPORTED_LANGUAGE_CODES = Set.of("cs_cz", "es_ar", "es_cl", "es_ec", "es_es", "es_mx", "es_uy", "es_ve", "fr_ca", "nl_be", "nl_nl", "ro_ro", "tr_tr", "zh_cn");
    ...
    private void addStarToSupportedLanguages(DrawContext instance, TextRenderer textRenderer, Text text, int centerX, int y, int color, Operation<Void> original) {
        if (SUPPORTED_LANGUAGE_CODES.contains(this.languageCode)) {
            instance.drawCenteredTextWithShadow(textRenderer, Text.literal("★ ").formatted(Formatting.YELLOW).append(text), centerX, y, color);
        } else if (SEMI_SUPPORTED_LANGUAGE_CODES.contains(this.languageCode)) {
            instance.drawCenteredTextWithShadow(textRenderer, Text.literal("★ ").formatted(Formatting.GOLD).append(text), centerX, y, color);
        } else {
