...
public class GrassEmptyChunkGenerator extends ChunkGenerator {
    public static final MapCodec<GrassEmptyChunkGenerator> CODEC = RecordCodecBuilder.mapCodec(instance ->
        instance.group(
        ...
        ).apply(instance, GrassEmptyChunkGenerator::new)
    );
    ...
    public static final int MIN_GENERATION_Y = -64;

    public GrassEmptyChunkGenerator(String structureName, int structureX, int structureZ, RegistryEntry<Biome> biome) {
        ...
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                ...
                for (int y = MIN_GENERATION_Y; y <= spikeHeight; y++) {
                    ...
                    } else if (y == spikeHeight) {
                        state = Blocks.GRASS_BLOCK.getDefaultState();
                    } else if (y > spikeHeight - 8) {
    ...
    private static int getSpikeHeight(int x, int z) {
        ...
        double spikeStrength = Math.min((v - 0.55) / 0.3, 1.0);
