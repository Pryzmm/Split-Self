...
import com.pryzmm.splitself.world.structure.pieces.BurialPiece;
...
public class BurialStructure extends Structure {
    public static final MapCodec<BurialStructure> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(Structure.configCodecBuilder(instance)).apply(instance, BurialStructure::new));

    public BurialStructure(Config config) {
        super(config);
    }
    ...
    public Optional<StructurePosition> getStructurePosition(Context context) {
        return Structure.getStructurePosition(context, Heightmap.Type.WORLD_SURFACE_WG, collector -> {
            ...
            collector.addPiece(new BurialPiece(context.structureTemplateManager(), finalPos));
        });
    }

    @Override
    public StructureType<?> getType() {
        return Structures.BURIAL;
    }
