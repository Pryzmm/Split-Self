...
public class PillarPiece extends SimpleStructurePiece {
    public static final String PIECE = "es_pillar";
    ...
    public PillarPiece(StructureTemplateManager manager, BlockPos pos) {
        super(StructurePieces.PILLAR, 0, manager, TEMPLATE_ID,
            TEMPLATE_ID.toString(), createPlacementData(), pos);

        try {
            String resourcePath = "/data/" + SplitSelf.MOD_ID + "/structures/" + PIECE + ".nbt";
            InputStream inputStream = PillarPiece.class.getResourceAsStream(resourcePath);
            if (inputStream != null) {
            ...
            SplitSelf.LOGGER.error("Failed to read pillar template size: {}", e.getMessage());
        }
    }

    public PillarPiece(StructureTemplateManager manager, NbtCompound nbt) {
        super(StructurePieces.PILLAR, nbt, manager, id -> createPlacementData());
    }
