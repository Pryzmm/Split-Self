...
public class FinalEscapePiece extends SimpleStructurePiece {
    public static final String PIECE = "final_escape";
    ...
    public FinalEscapePiece(StructureTemplateManager manager, BlockPos pos) {
        super(StructurePieces.FINAL_ESCAPE, 0, manager, TEMPLATE_ID,
            TEMPLATE_ID.toString(), createPlacementData(), pos);

        try {
            String resourcePath = "/data/" + SplitSelf.MOD_ID + "/structure/" + PIECE + ".nbt";
            InputStream inputStream = FinalEscapePiece.class.getResourceAsStream(resourcePath);
            if (inputStream != null) {
            ...
            SplitSelf.LOGGER.error("Failed to read final escape template size: {}", e.getMessage());
        }
    }

    public FinalEscapePiece(StructureTemplateManager manager, NbtCompound nbt) {
        super(StructurePieces.FINAL_ESCAPE, nbt, manager, id -> createPlacementData());
    }
