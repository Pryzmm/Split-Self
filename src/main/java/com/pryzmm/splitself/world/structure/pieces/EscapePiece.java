...
public class EscapePiece extends SimpleStructurePiece {
    public static final String PIECE = "es_escape";
    ...
    public EscapePiece(StructureTemplateManager manager, BlockPos pos) {
        super(StructurePieces.ESCAPE, 0, manager, TEMPLATE_ID,
            TEMPLATE_ID.toString(), createPlacementData(), pos);

        try {
            String resourcePath = "/data/" + SplitSelf.MOD_ID + "/structures/" + PIECE + ".nbt";
            InputStream inputStream = EscapePiece.class.getResourceAsStream(resourcePath);
            if (inputStream != null) {
            ...
            SplitSelf.LOGGER.error("Failed to read escape template size: {}", e.getMessage());
        }
    }

    public EscapePiece(StructureTemplateManager manager, NbtCompound nbt) {
        super(StructurePieces.ESCAPE, nbt, manager, id -> createPlacementData());
    }
