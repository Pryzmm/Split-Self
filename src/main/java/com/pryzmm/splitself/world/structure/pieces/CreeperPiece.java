...
public class CreeperPiece extends SimpleStructurePiece {
    public static final String PIECE = "es_creeper";
    ...
    public CreeperPiece(StructureTemplateManager manager, BlockPos pos) {
        super(StructurePieces.CREEPER, 0, manager, TEMPLATE_ID,
            TEMPLATE_ID.toString(), createPlacementData(), pos);

        try {
            String resourcePath = "/data/" + SplitSelf.MOD_ID + "/structures/" + PIECE + ".nbt";
            InputStream inputStream = CreeperPiece.class.getResourceAsStream(resourcePath);
            if (inputStream != null) {
            ...
            SplitSelf.LOGGER.error("Failed to read creeper template size: {}", e.getMessage());
        }
    }

    public CreeperPiece(StructureTemplateManager manager, NbtCompound nbt) {
        super(StructurePieces.CREEPER, nbt, manager, id -> createPlacementData());
    }
