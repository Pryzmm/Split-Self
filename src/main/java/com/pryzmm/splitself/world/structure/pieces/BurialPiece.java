...
public class BurialPiece extends SimpleStructurePiece {
    public static final String PIECE = "es_burial";
    ...
    public BurialPiece(StructureTemplateManager manager, BlockPos pos) {
        super(StructurePieces.BURIAL, 0, manager, TEMPLATE_ID,
            TEMPLATE_ID.toString(), createPlacementData(), pos);

        try {
            String resourcePath = "/data/" + SplitSelf.MOD_ID + "/structures/" + PIECE + ".nbt";
            InputStream inputStream = BurialPiece.class.getResourceAsStream(resourcePath);
            if (inputStream != null) {
            ...
            SplitSelf.LOGGER.error("Failed to read burial template size: {}", e.getMessage());
        }
    }

    public BurialPiece(StructureTemplateManager manager, NbtCompound nbt) {
        super(StructurePieces.BURIAL, nbt, manager, id -> createPlacementData());
    }
